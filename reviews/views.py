...
def search_reviews(request):
    ...
    search_results = GameReview.objects.filter(
        approved=True).order_by('-rating')
...
def user_approved_reviews(request):
    ...
    here.
    """
    approved_reviews = GameReview.objects.filter(
...
def user_unapproved_reviews(request):
    ...
    here.
    """
    unapproved_reviews = GameReview.objects.filter(
...
def review_approval(request, pending_review_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            pending_review.save()
            messages.success(
                request, f'Review {pending_review.title} has been approved.'
                )
...
def create_review(request):
    ...
    Form which uses fields from the `GameReviewModel`. Once a review has been
    created, it will be put up for review by an admin and redirect the user
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Your review has been submitted and is pending approval by an admin."  # noqa
                )
...
def edit_review(request, review_id):
    ...
    if review.author != request.user:
        ...
        return redicrect('unauthorized.html')
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            messages.success(
                request,
                'Review successfuly updated and is now up for approval by an admin'  # noqa
                )
            return redirect('home')
...
def delete_review(request, review_id):
   ...
   A logged in user has the ability to delete their own
    ...
    if request.user == review.author or request.user.is_superuser:
        ...
        messages.error(
            request, "You don't have permission to delete that review."
            )
